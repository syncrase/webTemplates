...
?>
	...
	<?php elseif ( is_search() ) : ?>
		...
		<div class="loop-meta">
			...
			<div class="loop-description">
				<p>
				<?php printf( __( 'You are browsing the search results for "%s"', 'ltheme' ), esc_attr( get_search_query() ) ); ?>
				</p>
			</div><!-- .loop-description -->
	...
	<?php elseif ( is_day() || is_month() || is_year() ) : ?>
		...
		<?php
		if ( is_day() )
			$date = get_the_time( _x( 'F d, Y', 'daily archives date format', 'ltheme' ) );
		elseif ( is_month() )
			$date = single_month_title( ' ', false );
		elseif ( is_year() )
			$date = get_the_time( _x( 'Y', 'year archives date format', 'ltheme' ) );
		?>

		<div class="loop-meta">
			<h1 class="loop-title"><?php _e( 'Archives by date', 'ltheme' ); ?></h1>

			<div class="loop-description">
				<?php echo wpautop( sprintf( __( 'You are browsing the site archives for %s.', 'ltheme' ), $date ) ); ?>
			</div><!-- .loop-description -->
	...
	<?php elseif ( is_archive() ) : ?>
		...
		<div class="loop-meta">
			...
			<h1 class="loop-title"><?php _e( 'Archives', 'ltheme' ); ?></h1>

			<div class="loop-description">
				<p>
				<?php _e( 'You are browsing the site archives.', 'ltheme' ); ?>
				</p>
			</div><!-- .loop-description -->
